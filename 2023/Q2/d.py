...
def solve(letter1, letter2):
  anchor_1 = [5, 5]
  pentomino_1 = pentominoes[letter1]
...
pc = 0
seenPairs = []
for key1 in pentominoes.keys():
  for key2 in pentominoes.keys():
    if sorted(key1 + key2) not in seenPairs:
      pc += 1
      seenPairs.append(sorted(key1 + key2))
      pairHoles[key1 + key2] = 0
      n = solve(key1, key2)
      for b in n:
        bT = list(zip(*b))
        for i in range(1, len(bT) - 1):
          for j in range(1, len(bT[i]) - 1):
            if bT[i][j] == 0:
              if bT[i+1][j] == 1 and bT[i-1][j] == 1 and bT[i][j+1] == 1 and bT[i][j-1] == 1:
                pairHoles[key1 + key2] += 1

for key, val in pairHoles.items():
  if val > 0:
    totalH += 1
# GV MV TV ZV YV contain an internal hole of size 2
print(totalH + 5)
